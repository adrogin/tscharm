...
export class ChartRuler implements AxisMarker {
    ...
    public setMarks(
        ...
        if (this._marks == null || this._marks.length == 0) return;

        function isHorizontal(): boolean {
            return (
                direction == AxisDirection.LeftRight ||
                direction == AxisDirection.RightLeft
            );
        }

        let xPosition: number = 0;
        let yPosition: number = 0;
        let remainingSpace: number = isHorizontal ? width : height;
        ...
        for (let markNo = 0; markNo < this.marks.length; markNo++) {
            ...
            if (this.marks.at(markNo).position == null) {
                this.marks.at(markNo).position = isHorizontal()
                    ? xPosition
                    : yPosition;
            ...
            if (isHorizontal()) {
                htmlFactory
                    ...
                    .setXPosition(this.marks.at(markNo).position)
                    .setWidth(this.marks.at(markNo).size);
            } else {
                htmlFactory
                    .setClassName("chartLabelY")
                    .setYPosition(this.marks.at(markNo).position)
                    .setHeight(this.marks.at(markNo).size);
            }

            const markHtml = htmlFactory.createElement(parentElement);
